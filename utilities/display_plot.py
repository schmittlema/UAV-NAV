...
class LivePlot(object):
    def __init__(self, outdir, data_key='episode_rewards',y ='cumulated episode rewards',title = 'averaged_simulation_graph',line_color='blue'):
        """
        ...
        #fig = plt.gcf().canvas.set_window_title(title)
    ...
    def plot(self, full=True, dots=False, average=0, interpolated=0):
        ...
        if full:
            ...
            graph = fig.add_subplot(111,xlabel='episodes',ylabel=self.y)
            graph.plot(data, color='blue')
        if dots:
        ...
        if interpolated > 0:
            ...
            #graph.plot(xx, interp(xx), color='green', linewidth=3.5)        
...
if __name__ == '__main__':
    ...
    length_plotter = LivePlot(outdir,'episode_lengths','episode lengths','episode_lengths')
