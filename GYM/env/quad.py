...
class GazeboQuadEnv(gazebo_env.GazeboEnv):
	...
	self.cur_state = rospy.ServiceProxy('/gazebo/get_model_state', GetModelState)
	...
	self.successes = 0
        self.current_position = ModelState()
	...
	while counter != 0:
        ...
        while not rospy.is_shutdown():
            self.get_state()
            des_vel = vController.update(self.current_position,self.x_vel,self.y_vel,self.current_position)
    ...
    def observe_test(self):
        rospy.wait_for_service('mavros/set_mode')
        return [self.current_position.pose.position.x - self.targetx]
    ...
    def detect_done(self,reward):
        ...
        dist_to_goal = abs(self.targetx-self.current_position.pose.position.x)
    ...
    def get_reward(self,action):
        direction = self.targetx-self.current_position.pose.position.x
